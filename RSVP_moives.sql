...
SELECT 
    year, 
    COUNT(*) AS movies_released_each_year
FROM
    movie
GROUP BY year;
...
FROM
	...
	role_mapping AS a
        ...
        movie AS m
            ...
            ratings AS r
				ON m.id=r.movie_id
WHERE lower(category) = 'actress' AND LOWER(languages) like '%hindi%'
...
SELECT 
    ...
    RANK() OVER (PARTITION BY g.genre, year
					ORDER BY CONVERT(REPLACE(TRIM(worlwide_gross_income), "$ ",""), UNSIGNED INT) DESC) 
                    AS movie_rank
FROM
movie AS m
	INNER JOIN
